...
The main program for twitter streaming was \textbf{tweepyCrawler.py}. It first starts by listening to the stream of data from twitters streaming API. Then once a a piece of data is received in JSON format parse the number of mementos. Twitter provides a key in JSON called ``urls'' which provided the urls of a tweet. The JSON could also have a``retweeted_status'' key which would point to the urls provided from the tweet referenced so I also included those urls.
...
Since I was streaming live tweets it was inevitable that I would get repeating, inappropriate, and blank URIs. Therefore, when receiving the JSON I would request to URI provided then check if the final URI was already in my output file, if it wasn't I would add it. I also added a blacklist of URIs and domain types that were in a the bad category. Sometimes this didn't already work, especially when it came to youtube URIs that had extra parameters inside them. To compensate for this I created \textbf{filterUnwanted.py} shown Listing \ref{lst:q1filter} in and \textbf{removeDuplicates.py} shown in Listing \ref{lst:q1duplicates}. The first script would strip youtube of their extra parameters except for their video ids and it would also remove bad URIs. The second script would make sure all the URIs were still unique in the file, removing duplicates if they were found.
...
For this problem I used  \url{http://memgator.cs.odu.edu/} to retrieve a JSON response for the time maps of my 1000 URIs. If the response was ``404 not found'' it was assumed that there were no mementos for the URI. As shown in Figure \ref{fig:q2histogram}

\begin{figure}[h]
\centering
\includegraphics[scale=0.5]{timemapHistogram.pdf}
